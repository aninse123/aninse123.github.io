...
(async () => {
  ...
  const ago = (d) => Timestamp.fromDate(new Date(Date.now() - d * DAY));
  ok("contacted (outreach) 10 days ago excluded, 40 days ago fine", why({ lastOutreachAt: ago(10) }).reason === "recent_touch" && why({ lastOutreachAt: ago(40) }).ok);
  ok("a recent note/meeting alone doesn't exclude (3c)", why({ lastTouchAt: ago(2), outreachAttempts: 0 }).ok);
  ok("older companies without lastOutreachAt: lastTouchAt counts only if they had outreach", why({ lastTouchAt: ago(2), outreachAttempts: 3 }).reason === "recent_touch");
  ok("lastOutreachAt wins over a more recent lastTouchAt", why({ lastOutreachAt: ago(60), lastTouchAt: ago(1), outreachAttempts: 2 }).ok);
  ok("recent-touch rule off at 0 days", why({ lastOutreachAt: Timestamp.now() }, { exclusions: { ...ctx.exclusions, contactedWithinDays: 0 } }).ok);
  ...
  seedCo("recent", { lastOutreachAt: Timestamp.now() });
